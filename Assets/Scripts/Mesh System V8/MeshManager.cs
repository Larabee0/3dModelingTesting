...
namespace MeshSystemV8
{
    ...
    public class MeshManager : MonoBehaviour
    {
        [Header("Mesh stuff")]
        [Header("Mesh Manager")]
        [SerializeField] private MeshFilter meshFilter; // Active MeshFilter
        ...
        [SerializeField] private MeshCollider Collider; // Active MeshCollider (technically its disabled but whatever)
        [Header("Internal Script References")]
        [HideInInspector] public MeshSelection meshSelection;
        ...
        public List<TriDataStore> SpawnedTriDS = new List<TriDataStore>();
         ...
         public bool LeftControlIsPressed = false;        //[HideInInspector]
         public bool LeftMouseButtonIsPressed = false;    //[HideInInspector]
        private void Start()
        {
        ...
        [SerializeField] private int[] VerticesToSimplifiedVerts;
        private void CreateSimplifiedVertices()
        {
            RelatedVertices.Clear();
            HashSet<Vector3> TempHasSet = new HashSet<Vector3>(LocalVertices);
            ...
            RelatedVertices.Capacity = LocalSimplifiedVerts.Count;
            ...
            //LocalVertices.ForEach(i => VerticesToSimplifiedVerts.Add(-1));
            VerticesToSimplifiedVerts = new int[LocalVertices.Count];
            Dictionary<Vector3, SimpVertContainer> RelatedVertDict = new Dictionary<Vector3, SimpVertContainer>(LocalSimplifiedVerts.Count);
            for (int i = 0; i < LocalSimplifiedVerts.Count; i++)
            {
                RelatedVertDict.Add(LocalSimplifiedVerts[i], new SimpVertContainer(new List<int>(),i));
            }
            for (int i = 0; i < LocalVertices.Count; i++)
            {
                RelatedVertDict[LocalVertices[i]].RelatedVerts.Add(i);
                VerticesToSimplifiedVerts[i] = RelatedVertDict[LocalVertices[i]].Index;
            }
            RelatedVertDict.Values.ToList().ForEach(i => RelatedVertices.Add(i.RelatedVerts));
        }
        ...
        private void CreateSimpliedTriangles()
        {
            ...
            Triangles.ForEach(i =>
            {
                SimplifiedTriangles.Add(VerticesToSimplifiedVerts[i]);
            });
            VerticesToSimplifiedVerts = null;
        }
        ...
        private void SpawnVerticeVisuals()
        {
            ...
            SpawnedVerts.Capacity = LocalSimplifiedVerts.Count;
            SpawnedVertsDS.Capacity = LocalSimplifiedVerts.Count;
            for (int i = 0; i < LocalSimplifiedVerts.Count; i++)
            {
                InstantiateVertex(transform.TransformPoint(LocalSimplifiedVerts[i])).RelatedVerts = RelatedVertices[i];
            }
        }

        private VertDataStore InstantiateVertex(Vector3 Pos)
        {
            GameObject VertexMesh = Instantiate(VertVisualElement, Pos, Quaternion.identity, vertContainer.transform);
            VertDataStore VDStore = VertexMesh.AddComponent<VertDataStore>();
            VertexMesh.name = (VDStore.SimpliedVertIndex = SpawnedVerts.Count).ToString();
        ...
        private void SpawnEdgeVisuals()
        {
            ...
            SpawnedEdges.Capacity = SimplifiedEdges.Count;
            SpawnedEdgesDS.Capacity = SimplifiedEdges.Count;
            for (int i = 0; i < SimplifiedEdges.Count; i++)
            {
                InstantiateEdge(SpawnedVertsDS[SimplifiedEdges[i][0]], SpawnedVertsDS[SimplifiedEdges[i][1]]);
            }
        }

        private EdgeDataStore InstantiateEdge(VertDataStore originVDS, VertDataStore newVertVDS)
        {
            ...
            EdgeMesh.name = (EDStore.EdgePositionsIndex = SpawnedEdges.Count).ToString();
            ...
            originVDS.InvolvedEdges.Add(EDStore); newVertVDS.InvolvedEdges.Add(EDStore); SpawnedEdgesDS.Add(EDStore);
        ...
        private void SpawnTriangleVisuals()
        {
            ...
            SpawnedTris.Capacity = SimplifiedTriangles.Count;
            SpawnedTriDS.Capacity = SimplifiedTriangles.Count;
            for (int i = 0; i < SimplifiedTriangles.Count; i += 3)
            {
                InstantiateTriangle(new VertDataStore[]
                {
                    SpawnedVertsDS[SimplifiedTriangles[i]],
                    SpawnedVertsDS[SimplifiedTriangles[i + 1]],
                    SpawnedVertsDS[SimplifiedTriangles[i + 2]] 
                }
                ).TriangleArrayInfo.AddRange(new List<int> { Triangles[i], Triangles[i + 1], Triangles[i + 2] });
            }
        }

        private TriDataStore InstantiateTriangle(VertDataStore[] VDS)
        {
            ...
            TriangleMesh.name = (TriDStore.TriIndex = SpawnedTris.Count).ToString();
        ...
        public void GetAdjacentTris()
        {
            SpawnedTriDS.ForEach(i => i.InvolvedTriDS.Clear());
        ...
        private VertDataStore NewVertex(VertDataStore Origin)
        {
            ...
            VertDataStore VDStore = InstantiateVertex(Origin.transform.position);
        ...
        public List<GameObject> ExtrudeSection(List<GameObject> OriginTris)
        {
            ...
            List<EdgeDataStore> NewEdges = new List<EdgeDataStore>();
            for (int i = 0; i < UnquieVertOrigin.Count; i++)
            {
                NewVerts.Add(ExtrudeVertex(UnquieVertOrigin[i]));
                NewEdges.Add(NewEdge(UnquieVertOrigin[i], NewVerts[i]));
            }
            ...
            {
                NewEdges.Add(NewEdge(NewVerts[UnquieVertOrigin.IndexOf(DirectVertexEdgeMap[i])], NewVerts[UnquieVertOrigin.IndexOf(DirectVertexEdgeMap[i + 1])]));
            }

            List<EdgeDataStore> MatchingEdges = new List<EdgeDataStore>();

            // This is terrible, use the connected triangles or involved triangle or whatever the fuck its called. the triangles store a list of triangles they're directly connected to.
            // this would massively improve the efficiecny of this for loop instead of checking through all teh edges
            bool breakout = false;
            ...
            List<HashSet<VertDataStore>> ExcudeEdgesBetween = new List<HashSet<VertDataStore>>();
            for (int i = 0; i < MatchingEdges.Count; i++)
            {
                ExcudeEdgesBetween.AddRange(new List<HashSet<VertDataStore>> { new HashSet<VertDataStore> { MatchingEdges[i].InvolvedVertObjects[0], MatchingEdges[i].InvolvedVertObjects[1] }, new HashSet<VertDataStore> { MatchingEdges[i].InvolvedVertObjects[1], MatchingEdges[i].InvolvedVertObjects[0] } });
            }
            ...
            for (int i = 0; i < TenativeFoldOuts.Count; i++)
            {
                ...
                NewEdges.Add(NewEdge(VertRemap[i][0], TenativeFoldOuts[i][1]));
            }
            ...
            {
                if(NewEdges[i].InvolvedTriangles.Count == 0)
                {
                    for (int k = 0; k < NewEdges[i].InvolvedVertObjects.Count; k++)
                    {
                        NewEdges[i].InvolvedVertObjects[k].InvolvedEdges.Remove(NewEdges[i]);
                        for (int j = 0; j < NewEdges[i].InvolvedVertObjects.Count; j++)
                        {
                            if (NewEdges[i].InvolvedVertObjects[j] != NewEdges[i].InvolvedVertObjects[k])
                            {
                                NewEdges[i].InvolvedVertObjects[k].ConnectedVerts.Remove(NewEdges[i].InvolvedVertObjects[j]);
                            }
                        }
                    }
                    SpawnedEdgesDS.Remove(NewEdges[i]);
                    SpawnedEdges.Remove(NewEdges[i].gameObject);
                    Destroy(NewEdges[i].gameObject);
                }
            }
        ...
        private EdgeDataStore SpawnEdge(VertDataStore originVDS, VertDataStore newVertVDS)
        {
            ...
            EdgeDataStore EDStore = InstantiateEdge(originVDS, newVertVDS);
            EDStore.Start();
            return EDStore;
        }
        ...
        private TriDataStore NewTri(VertDataStore[] VertDS)
        {
            ...
            TriDataStore triDataStore = InstantiateTriangle(VertDS);
            triDataStore.TriangleArrayInfo.AddRange(NewTriBuilder);
            triDataStore.Position = (VertLocalPositions[0] + VertLocalPositions[1] + VertLocalPositions[2]) / 3;
            return triDataStore;
        }
        #endregion
        ...
        public void RemoveEdge(GameObject ToRemove)
        {
            ...
            List<TriDataStore> InvolvedTriangles = ToRemoveEDS.InvolvedTriangles.ToList();
            for (int i = 0; i < InvolvedTriangles.Count; i++)
            {
                RemoveTriangle(InvolvedTriangles[i]);
            }
        ...
        private void DeleteTri(TriDataStore ToRemoveTDS)
        {
            ...
            for (int i = 0; i < ToRemoveTDS.InvolvedEdgeDS.Count; i++)
            {
                if(ToRemoveTDS.InvolvedEdgeDS[i].InvolvedTriangles.Count == 0)
                {
                    ...
                    {
                        ToRemoveTDS.InvolvedEdgeDS[i].InvolvedVertObjects[k].InvolvedEdges.Remove(ToRemoveTDS.InvolvedEdgeDS[i]);
                        for (int j = 0; j < ToRemoveTDS.InvolvedEdgeDS[i].InvolvedVertObjects.Count; j++)
                        {
                            if(ToRemoveTDS.InvolvedEdgeDS[i].InvolvedVertObjects[j] != ToRemoveTDS.InvolvedEdgeDS[i].InvolvedVertObjects[k])
                            {
                                ToRemoveTDS.InvolvedEdgeDS[i].InvolvedVertObjects[k].ConnectedVerts.Remove(ToRemoveTDS.InvolvedEdgeDS[i].InvolvedVertObjects[j]);
                            }
                        }
                    }
                    SpawnedEdgesDS.Remove(ToRemoveTDS.InvolvedEdgeDS[i]);
                    SpawnedEdges.Remove(ToRemoveTDS.InvolvedEdgeDS[i].gameObject);
            ...
            for (int i = 0; i < ToRemoveTDS.InvolvedVertObjects.Count; i++)
            {
                if(ToRemoveTDS.InvolvedVertObjects[i].ConnectedVerts.Count == 1)
                {
                    SpawnedVertsDS.Remove(ToRemoveTDS.InvolvedVertObjects[i]);
                    SpawnedVerts.Remove(ToRemoveTDS.InvolvedVertObjects[i].gameObject);
                    Destroy(ToRemoveTDS.InvolvedVertObjects[i].gameObject);
                }
            }
        ...
        public void StartEverything()
        {
            ...
            float timerTwo = Time.realtimeSinceStartup;
            ...
            isCloned = true;
            print("clone time: " + (Time.realtimeSinceStartup - timerTwo) * 1000f + "ms");
            timerTwo = Time.realtimeSinceStartup;
            LocalVertices = new List<Vector3>(clonedMesh.vertices);
            print("LocalVertices list time: " + (Time.realtimeSinceStartup - timerTwo) * 1000f + "ms");
            timerTwo = Time.realtimeSinceStartup;
            Triangles = new List<int>(clonedMesh.triangles);
            SimplifiedTriangles.Capacity = Triangles.Count;
            print("Triangles list time: " + (Time.realtimeSinceStartup - timerTwo) * 1000f + "ms");
            timerTwo = Time.realtimeSinceStartup;
            RebuildVisualControls();

            print("RebuildVisualControls time: " + (Time.realtimeSinceStartup - timerTwo) * 1000f + "ms");
            isCloned = true;
        ...
        private void RebuildVisualControls()
        {
            ...
            print("ConvertLocalToGlobal() Time: " + (Time.realtimeSinceStartup - startTime) * 1000f + "ms");

            startTime = Time.realtimeSinceStartup;
            CreateSimplifiedVertices();
            print("CreateSimplifiedVertices() Time: " + (Time.realtimeSinceStartup - startTime) * 1000f + "ms");
            startTime = Time.realtimeSinceStartup;
            CreateSimpliedTriangles();
            print("CreateSimpliedTriangles() Time: " + (Time.realtimeSinceStartup - startTime) * 1000f + "ms");
            startTime = Time.realtimeSinceStartup;
            CreateSimpliedEdges();
            print("CreateSimpliedEdges() Time: " + (Time.realtimeSinceStartup - startTime) * 1000f + "ms");

            startTime = Time.realtimeSinceStartup;
            SpawnVerticeVisuals();
            print("SpawnVerticeVisuals() Time: " + (Time.realtimeSinceStartup - startTime) * 1000f + "ms");
            startTime = Time.realtimeSinceStartup;
            SpawnEdgeVisuals();
            print("SpawnEdgeVisuals() Time: " + (Time.realtimeSinceStartup - startTime) * 1000f + "ms");
            startTime = Time.realtimeSinceStartup;
            SpawnTriangleVisuals();
            print("SpawnTriangleVisuals() Time: " + (Time.realtimeSinceStartup - startTime) * 1000f + "ms");

            startTime = Time.realtimeSinceStartup;
            GetAdjacentTris();
            print("GetAdjacentTris() Time: " + (Time.realtimeSinceStartup - startTime) * 1000f + "ms");

            startTime = Time.realtimeSinceStartup;
            meshSelection.OnMSEnable();
            print("meshSelection.OnMSEnable() Time: " + (Time.realtimeSinceStartup - startTime) * 1000f + "ms");
        }
        #endregion
        ...
        public void UpdateMeshAndOptimizeStartStop()
        {
            clonedMesh.vertices = LocalVertices.ToArray();
            clonedMesh.triangles = Triangles.ToArray();
            clonedMesh.RecalculateNormals();
        ...
        private void UpdateMeshAndOptimize()
        {
            clonedMesh.vertices = LocalVertices.ToArray();
            clonedMesh.triangles = Triangles.ToArray();
            clonedMesh.RecalculateNormals();
        ...
        public void ConvertLocalToGlobal()
        {
            ...
            {
                GlobalVertices.Capacity = LocalVertices.Count;
                LocalVertices.ForEach(i => GlobalVertices.Add(transform.TransformPoint(i)));
            }
            else
            {
                for (int i = 0; i < LocalVertices.Count; i++)
        ...
        private void OnGUI()
        {
            ...
            if (isCloned)
            {
                if (GUI.Button(new Rect(10, 0, 120, 30), "Optimize Mesh"))
                {
                    UpdateMeshAndOptimizeStartStop();
                }
                if (GUI.Button(new Rect(10, 35, 120, 30), "Log Relation Lists"))
                {
                    LogRelationLists();
                }
            }
