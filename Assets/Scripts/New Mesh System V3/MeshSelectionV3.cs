...
public class MeshSelectionV3 : MonoBehaviour
{
    ...
    public GameObject FirstReturnedItem;
    ...
    void Update()
    {
        ...
        else if (Input.GetKeyDown(KeyCode.I)) // Clones splits mesh into seperate triangle objects.
        {
            ...
            if (MM.TriangleComponents[0].EdgeCollidersEnabled)
            {
                ToggleEdges();
            }
            if (MM.TriangleComponents[0].TriCollidersEnabled)
            {
                ToggleFaces();
            }
        ...
        if(Mode == 2 && PrimarySelectedObjectTri != null)
        {
            TriReTransformHandles();
        }
        ...
        if (MM.TrianglesObjects.Count - 1 >= 0)
        {
            ...
            {
                if (!MM.TriangleComponents[0].VertCollidersEnabled)
                {
                    if (MM.TriangleComponents[0].EdgeCollidersEnabled)
                    {
                        ToggleEdges();
    ...
    void ToggleEdges()
    {
        if (MM.TriangleComponents[0].EdgeCollidersEnabled)
        {
            ...
            {
                MM.TriangleComponents[i].DisableEdgeColliders();
            }
        }
        else
        {
            for (int i = 0; i < MM.TriangleComponents.Count; i++)
            {
                MM.TriangleComponents[i].EnableEdgeColliders();
            }
        }
    }
    void ToggleVertices()
    {
        if (MM.TriangleComponents[0].VertCollidersEnabled)
        {
            for (int i = 0; i < MM.TriangleComponents.Count; i++)
            {
                MM.TriangleComponents[i].DisableVertColliders();
            }
        }
        else
        {
            for (int i = 0; i < MM.TriangleComponents.Count; i++)
            {
                MM.TriangleComponents[i].EnableVertColliders();
            }
        }
    }
    ...
    void ToggleFaces()
    {
        if (MM.TriangleComponents[0].TriCollidersEnabled)
        {
            ...
            for (int i = 0; i < MM.TriangleComponents.Count; i++)
            {
                MM.TriangleComponents[i].DisableTriColliders();
            }
        }
        else
        {
            ...
            for (int i = 0; i < MM.TriangleComponents.Count; i++)
            {
                MM.TriangleComponents[i].EnableTriColliders();
            }
        }
    }
    #endregion
    ...
    void ConvertGlobalSelectionToLocal()
    {
        ...
        //FirstReturnedItem = Results[Results.Count - 1];
        //Debug.Log()
    ...
    void SelectionBackEnd(GameObject Item, bool BoxSelect)
    {
        ...
        if (PrimarySelectedObjectVertex == null)
        {
            ...
            else if (Mode == 2) // Face Selection
            {
                ...
                if (MM.SpawnedHandle == null)
                {
                    MM.InstantiateHandle(PrimarySelectedObjectTri.transform, PrimarySelectedObjectTri.GetComponentInChildren<Triangle2>().EdgeAveragePositon);
                }
                else
                {
                    MM.ReTransformHandle(PrimarySelectedObjectTri.transform, PrimarySelectedObjectTri.GetComponentInChildren<Triangle2>().EdgeAveragePositon);
                }
            }
        }
        else
    ...
    public void TriReTransformHandles()
    {
        if (MM.SpawnedHandle == null)
        {
            MM.InstantiateHandle(PrimarySelectedObjectTri.transform, PrimarySelectedObjectTri.GetComponentInChildren<Triangle2>().EdgeAveragePositon);
